...
class LLMGenerationManager:
    def __init__(
        ...
        self.retriever = BM25(tokenizer=self.tokenizer, index_name='wiki')
    ...
    def run_llm_loop(self, gen_batch, initial_input_ids: torch.Tensor) -> Tuple[Dict, Dict]:
        ...
        active_mask = torch.ones(gen_batch.batch['input_ids'].shape[0], dtype=torch.bool) # prompt对应的 response有没有结束
        turns_stats = torch.ones(gen_batch.batch['input_ids'].shape[0], dtype=torch.int) # 每个prompt对应的response的turn数
        ...
        for step in range(self.config.max_turns):
            print(f"total {gen_batch.batch['input_ids'].shape[0]} responses, remaining {active_mask.sum().item()} responses")
            ...
            # 这个函数是为了dummy pad generate
            # 生成follow up 
            # TODO 加 stop str， let's search
            rollings_active.meta_info['stop'] = ["Let's search"]
    ...
    def execute_predictions(self, predictions: List[str], pad_token: str, active_mask=None, do_search=True) -> List[str]:
        ...
        # TODO action：retrieve/ no retrieve(final answer)
        ...
        if do_search:
            ...
            search_results = self.batch_bm25_search(search_queries)
        ...
        for i, (action, active) in enumerate(zip(cur_actions, active_mask)):
            ...
            if not active:
                ...
                if action == 'answer':
                    ...
                    next_obs.append(f" the question in Wikipedia.\n{search_results.pop(0).strip()}\nIntermediate answer:")
                    dones.append(0)
                    valid_action.append(1)
                    is_search.append(1)
                else:
...
#                     is_search.append(0)
    ...
    def postprocess_predictions(self, predictions: List[Any]) -> Tuple[List[int], List[bool]]:
        ...
        for pred in predictions:
            ...
            if pred.endswith("Let's search"):
                actions.append("search")
                contents.append(pred.removesuffix("Let's search").strip().split("Follow up:")[-1].strip())  # Will be filled from Follow up
            else:
                actions.append("answer")
                contents.append(pred)
    ...
    def batch_bm25_search(self, queries: List[str]):
        _, docs_list = self.retriever.retrieve(queries, topk=self.config.topk)
